...
$(document).ready(function() {
 ...
 //    });
 	...
 	$(document).on('click touch' ,'.ajax-addtocart' ,function(e){
		e.preventDefault();
		let _this = $(this);
		...
		if(condition == 'true'){
			...
			$.ajax({
                ...
                data : {
                    ...
                    promotionalid: param.promotionalid, 
                    id: param.id
                },
 ...
 $(window).load(function() {
	...
	$(document).on('click touch' ,'.js_buy' ,function(e){
		e.preventDefault();
		render_price();
		let _this = $(this);
...
$(document).ready(function () {
    $(document).on('click','.js_catalogue',function(){
    	let _this = $(this)
    	let catalogueid = _this.attr('data-catalogueid');
    	let image = _this.attr('data-image');

    	$('.banner-panel .image').find('img').attr('src', image);


   		let ajaxURL = 'homepage/ajax/home/index';
		$.get(ajaxURL, {
			catalogueid : catalogueid},
			function(data){
	...
	$(document).on('click','.js_search',function(){
    	let _this = $(this)
    	let catalogueid = _this.parents('form').find('select[name=catalogueid]').val()
    	let keyword = _this.parents('form').find('input[name=keyword]').val()
   		let ajaxURL = 'homepage/ajax/home/index';
		$.get(ajaxURL, {
			catalogueid : catalogueid, keyword : keyword},
			function(data){
				$('.js_content').html(data)	
		});
	...
	$(document).on('click','.js_pagination li a', function(e){
		e.preventDefault(); 
		//lấy dữ liệu url
		// let obj = get_obj_in_url();
		// convert_false_to(obj, {})
		let page = $(this).attr('data-ci-pagination-page');
		let catalogueid = $('.js_search').parents('form').find('select[name=catalogueid]').val()
    	let keyword = $('.js_search').parents('form').find('input[name=keyword]').val()
    	console.log(page)

		let ajaxURL = 'homepage/ajax/home/index/'+page;
		$.get(ajaxURL, {
			catalogueid : catalogueid, keyword : keyword},
			function(data){
				$('.js_content').html(data)	
