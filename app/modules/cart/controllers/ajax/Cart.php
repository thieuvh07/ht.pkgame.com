...
class Cart extends MY_Controller {
    ...
    public function send_otp(){
        ...
        if($this->input->post('phone')){
            $_number = $this->input->post('phone');
            //gọi thư viện validation
            $this->load->library('form_validation');
            $this->form_validation->set_error_delimiters(' ' , ' /');
            $this->form_validation->set_rules('phone' , 'Số điện thoại' , 'trim|required|min_length[10]|max_length[10]');
            
            if($this->form_validation->run($this)){
                // gọi api
                require("plugin/sms/SpeedSMSAPI.php");
                $smsAPI = new SpeedSMSAPI("e2-cALbMf6S4gDmrhzHUPzDV0qefTF6I");
                $userInfo = $smsAPI->getUserInfo();
                $phones = [$_number]; // số điện thoại nhận otp
                /* tối đa 100 số cho 1 lần gọi API */
                //tạo mã otp
                $otpCode = random_cart(6);
                
                $content = "Mã xác nhận cho đơn hàng của bạn là: ".$otpCode;
                
                // lưu ss, coki
                // 1.coki , thời gian gửi otp + 5p
                // 2. ng dùng đăng nhập thì lưu vào db, trong 90 ngày thì xóa (mô hình tmdt tiki, ...)
                
                $_SESSION['otp_code'] = $otpCode;
                // echo '<pre>';
                // print_r($_SESSION); die;
                
                $type = 2; //2: tin nhắn gửi bằng đầu số ngẫu nhiên
                $sender = $this->general['homepage_company'];
                ...
                if($response['status'] == 'error'){
                    if($response['code'] == 105){
                        $error = array(
                            'flag' => 1,
                            'message' => 'Số điện thoại '.$response['invalidPhone'][0].' không đúng. Xin vui lòng kiểm tra lại',
                        );
                    }else{
                        $error = array(
                            'flag' => 1,
                            'message' => 'Đã có lỗi trong quá trình gửi OTP, xin vui lòng kiểm tra lại',
                        );
                    }
                }else{
                    $error = array(
    ...
    public function _CheckPhoneNumber($phoneNumber = ''){
        $pattern = '/0[1-9]{1}[0-9]{8}/'; //mẫu
        if(!preg_match($pattern, $phoneNumber, $matches)){
            $this->form_validation->set_message('_CheckPhoneNumber' , 'Số điện thoại không đúng');
            return false;
        }
        return true;
    }
	...
	public function addCart(){
        ...
        $name = isset($post['name'])? $post['name'] : 'Sản phẩm';
        $attrids = isset($post['attrids'])? '-attrids-'.$post['attrids'] : '';
        $promotionalid = isset($post['promotionalid'])? str_replace(',','-',$post['promotionalid']) : '';
        $content = isset($post['content'])? str_replace(',','-',$post['content']) : '';



		$data = array(
            "id" => 'SKU-prd-'.$id.$attrids,
            "name" => cutnchar($name, 15),
            "qty" => $quantity,
            "price" => $price,
            "option" => array(
            	"id" => $id, 
