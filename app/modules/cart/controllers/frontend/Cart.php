...
class Cart extends MY_Controller {
	...
	public function payment(){
		...
		if($this->input->post('create')){
			...
			$this->form_validation->set_rules('otp_code', 'Mã OTP', 'trim|required|is_numeric|min_length[6]|max_length[6]|callback__CheckOTP');
			if($this->form_validation->run($this)){
				...
				$extend['phone_receive'] = $this->input->post('phone');
	...
	public function _CheckOTP($otpCode = 0){
		$otpSMS = $_SESSION['otp_code'];
		if($otpCode != $otpSMS){
			$this->form_validation->set_message('_CheckOTP','Mã OTP không chính xác');
			return false;
		}
		return true;
	}
