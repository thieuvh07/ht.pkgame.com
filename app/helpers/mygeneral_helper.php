<?php
if(!function_exists('random_cart')){
	function random_cart($leng = 168, $char = FALSE){
		if($char == FALSE) $s = '0123456789';
		else $s = '0123456789';
...
if(!function_exists('show_flashdata_frontend')){
	function show_flashdata_frontend($body = TRUE){
		...
		if(isset($message)){
			if($body == TRUE) $result = $result . '<div class="uk-container uk-container-center uk-margin-top"><div class="uk-alert uk-alert-success uk-margin-remove" data-uk-alert>';
		...
		if(isset($message)){
			if($body == TRUE) $result = $result . '<div class="uk-container uk-container-center uk-margin-top"><div class="uk-alert uk-alert-danger uk-margin-remove" data-uk-alert>';
			$result = $result . '<a href="#" class="uk-alert-close uk-close"></a><p>'.$message.'</p>';
			if($body == TRUE) $result = $result . '</div></div>';
			return $result;
		}
		...
		if(isset($jsonData) && is_array($jsonData) && count($jsonData)){
			foreach($jsonData as $key => $val){
				...
				$jsonData[$key][(count($module) == 1) ? 'post'  : 'catalogue'] = $CI->Autoload_Model->_get_where(array(
					'select' => 'id, title, slug, canonical, image, description'.((count($module) > 1) ? ', icon' : '').(($val['module'] == 'product_catalogue') ? ', image_json' : '').(($val['module'] == 'product') ? ', price, price_sale, price_contact' : ''),
					'table' => $val['module'],
					...
					if(isset($val['catalogue']) && is_array($val['catalogue']) && count($val['catalogue'])){
						foreach($val['catalogue'] as $keyChildren => $valChildren){
							$jsonData[$key]['catalogue'][$keyChildren]['children'] =  $CI->Autoload_Model->_get_where(array(
								'select' => 'id, title, slug, canonical, icon',
								'table' => $val['module'],
